"use client";

import { useEffect, useState } from "react";
import {
  AreaChart,
  Area,
  ResponsiveContainer,
  Tooltip,
  XAxis,
  YAxis,
} from "recharts";
import { Card, CardHeader, CardTitle, CardContent, CardDescription } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Skeleton } from "@/components/ui/skeleton";
import { useMarketTokens } from "@/hooks/use-market-tokens";
import { useSolanaStats } from "@/hooks/use-solana-stats";
import {
  fetchSolanaPriceChart,
  type MarketChartPoint,
} from "@/lib/market-data";
import { formatCompact, formatPrice } from "@/lib/utils";

export function MarketPulse() {
  const [data, setData] = useState<MarketChartPoint[]>([]);
  const [loading, setLoading] = useState(true);
  const { stats: sol } = useSolanaStats();
  const { tokens } = useMarketTokens();

  useEffect(() => {
    let cancelled = false;
    const controller = new AbortController();
    async function load() {
      try {
        const points = await fetchSolanaPriceChart(controller.signal);
        if (cancelled) return;
        setData(points);
      } finally {
        if (!cancelled) setLoading(false);
      }
    }
    load();
    const interval = window.setInterval(load, 60_000);
    return () => {
      cancelled = true;
      controller.abort();
      window.clearInterval(interval);
    };
  }, []);

  const last = data.length > 0 ? data[data.length - 1].v : sol?.priceUsd ?? 0;
  const first = data.length > 0 ? data[0].v : last / (1 + (sol?.change24h ?? 0) / 100);
  const change = first > 0 ? ((last - first) / first) * 100 : sol?.change24h ?? 0;

  const aggVolume = tokens.reduce((sum, t) => sum + t.volume24h, 0);
  const aggLiquidity = tokens.reduce((sum, t) => sum + t.liquidity, 0);
  const avgChange = tokens.length > 0
    ? tokens.reduce((sum, t) => sum + t.change24h, 0) / tokens.length
    : 0;

  return (
    <Card className="lg:col-span-2 overflow-hidden">
      <CardHeader className="flex flex-row items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <CardTitle>Market pulse</CardTitle>
            <Badge variant="live" className="text-[9px]">LIVE</Badge>
          </div>
          <CardDescription>
            SOL price · 24h · CoinGecko
          </CardDescription>
        </div>
        <div className="text-right">
          <div className="text-2xl font-semibold tracking-tight">
            {loading ? <Skeleton className="h-7 w-24" /> : formatPrice(last)}
            <span className="text-sm text-muted-foreground ml-1">SOL</span>
          </div>
          <div
            className={`text-xs font-mono ${change >= 0 ? "text-emerald-400" : "text-rose-400"}`}
          >
            {change >= 0 ? "+" : ""}
            {change.toFixed(2)}%
          </div>
        </div>
      </CardHeader>
      <CardContent className="px-1 pt-1 pb-2">
        <div className="h-56">
          {loading && data.length === 0 ? (
            <div className="h-full flex items-center justify-center px-4">
              <Skeleton className="h-full w-full rounded-md" />
            </div>
          ) : (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                <defs>
                  <linearGradient id="pulseFill" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="hsl(var(--neon-mint))" stopOpacity={0.5} />
                    <stop offset="100%" stopColor="hsl(var(--neon-mint))" stopOpacity={0} />
                  </linearGradient>
                </defs>
                <XAxis hide dataKey="t" />
                <YAxis hide domain={["auto", "auto"]} />
                <Tooltip
                  contentStyle={{
                    background: "rgba(10,12,18,0.95)",
                    border: "1px solid rgba(255,255,255,0.08)",
                    borderRadius: 10,
                    fontSize: 11,
                  }}
                  labelStyle={{ color: "hsl(var(--muted-foreground))" }}
                  formatter={(v: unknown) => [formatPrice(Number(v) || 0), "SOL"]}
                  labelFormatter={(label: unknown) =>
                    new Date(Number(label)).toLocaleTimeString()
                  }
                />
                <Area
                  type="monotone"
                  dataKey="v"
                  stroke="hsl(var(--neon-mint))"
                  strokeWidth={2}
                  fill="url(#pulseFill)"
                />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </div>
        <div className="grid grid-cols-3 gap-3 px-4 pt-3 pb-3 text-xs">
          <Stat
            label="DEX volume"
            value={aggVolume > 0 ? `$${formatCompact(aggVolume)}` : "—"}
            change={Number(avgChange.toFixed(2))}
          />
          <Stat
            label="Tracked liquidity"
            value={aggLiquidity > 0 ? `$${formatCompact(aggLiquidity)}` : "—"}
          />
          <Stat
            label="SOL market cap"
            value={sol ? `$${formatCompact(sol.marketCap)}` : "—"}
            change={sol ? Number(sol.change24h.toFixed(2)) : undefined}
          />
        </div>
      </CardContent>
    </Card>
  );
}

function Stat({
  label,
  value,
  change,
}: {
  label: string;
  value: string;
  change?: number;
}) {
  return (
    <div className="rounded-lg bg-white/[0.02] border border-white/[0.04] px-3 py-2">
      <div className="text-[10px] uppercase tracking-wider text-muted-foreground/70">
        {label}
      </div>
      <div className="mt-1 font-medium">{value}</div>
      {change !== undefined ? (
        <div
          className={`text-[10px] font-mono ${change >= 0 ? "text-emerald-400" : "text-rose-400"}`}
        >
          {change >= 0 ? "+" : ""}
          {change}%
        </div>
      ) : (
        <div className="text-[10px] font-mono text-muted-foreground/70">—</div>
      )}
    </div>
  );
}
